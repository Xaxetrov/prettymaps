    ...
    background = shapely.affinity.scale(
        box(
            *shapely.ops.unary_union(
                ox.project_gdf(gdfs["perimeter"]).geometry.buffer(background_dilate)
            ).bounds
        ),
        background_pad,
        background_pad,
...
def plot(
    ...
    if mode == "matplotlib":
        ...
        if title:
            fp = FontProperties(fname = '/home/edern/prettymaps/prettymaps/assets/PermanentMarker-Regular.ttf', size = 50)
            ax.set_title(
                label="Lyon, France",
                loc="center",
                y=0.97,
            ...
            plt.text(np.interp([0.5], [0, 1], [xmin, xmax])[0],np.interp([0.007], [0, 1], [ymin, ymax])[0],"45.76033, 4.84912", horizontalalignment='center', size=50, fontproperties = fp)
            plt.text(np.interp([0.02], [0, 1], [xmin, xmax])[0],np.interp([0.5], [0, 1], [ymin, ymax])[0],"Generated by Edern \"Xaxetrov\" Haumont on 2023/08/08", rotation='vertical', verticalalignment='center', size=16, fontproperties = fp)
            plt.text(np.interp([0.974], [0, 1], [xmin, xmax])[0],np.interp([0.5], [0, 1], [ymin, ymax])[0],"Credits to prettymaps by Marcelo Prates and OpenStreetMap contributors", rotation=270, verticalalignment='center', size=16, fontproperties = fp)
